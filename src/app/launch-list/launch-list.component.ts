...
export class LaunchListComponent implements OnInit {
  ...
  constructor(private spaceXService: SpaceXService, public router: Router) { }
  ...
  updateCarousel() {
    ...
    if (itemWidth) {
      carousel.style.transform = `translateX(-${this.currentIndex * (itemWidth + 40)}px)`; // 40px is the margin
    }
  }
  ...
  applyFilters() {
    this.filteredLaunches = this.launches.filter(launch => {
      return (!this.filterCriteria.mission_name || launch.mission_name.includes(this.filterCriteria.mission_name)) &&
             (!this.filterCriteria.launch_year || launch.launch_year === this.filterCriteria.launch_year) &&
             (this.filterCriteria.launch_success === null || launch.launch_success === this.filterCriteria.launch_success) &&
             (!this.filterCriteria.rocket_name || launch.rocket.rocket_name.includes(this.filterCriteria.rocket_name)) &&
             (!this.filterCriteria.site_name || launch.launch_site.site_name.includes(this.filterCriteria.site_name)) &&
             (this.filterCriteria.upcoming === null || launch.upcoming === this.filterCriteria.upcoming);
    });
  }
}
